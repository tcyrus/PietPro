...
Session = { set: function(key, val) { window.meteorSession[key] = val; }, get: function(key) { return window.meteorSession[key]; } };
  ...
  var colArr = [
    ...
    ['#FFC0FF', '#FF00FF', '#C000C0'],   //rendered sideways 
  ];
...
var commandArr = [
  ...
  function arrFind(arr, target){//returns [row, col]
  ...
  Template.main.events({
    ...
    'click td.ccell': function(event){
      ...
      Session.set('currentCol', rgbToHex(event.target.style.backgroundColor));
    ...
    'click td.fcell': function(event){
      ...
      colCoords = arrFind(colArr, Session.get('currentCol'));
